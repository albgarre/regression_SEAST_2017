...
Asignatura: Simulación estadística.
...
```{r}
## Completar aqui"
# cargamos librerías que necesitaremos más adelante
library(lubridate)
library(dplyr)
library(ggplot2)
...
```{r}
# Completar aquí
MediaVSPauDec <- GradosTelecoSimple %>%
  ggplot(aes(x = PAU_CALIFICACION_NOTA_MATERIA_DECIL, y = notamedia)) +
  geom_point()
MediaVSPauDec
```
...
En las últimas filas se ve como el coste se reduce a una velocidad muy lenta, disminuyendo su valor a partir del sexto decimal.
...
for (t in 1:length(alpha) )
  ...
  theta=thetainicial
  for(i in 1:maxiter){
    theta <- theta - alpha[t] * gradJ(theta)
  monitor[(i+1)+(3000*(t-1)), ] = c(i, theta, J(theta), gradJ(theta), alpha[t])
  }
}
```
...
costeVSiterALPHA <- monitor %>% 
  ggplot(aes(x = iter, y = J, color = alpha)) +
  geom_line() +
  geom_point()
costeVSiterALPHA
```
...
deltaJ <- 1
theta=thetainicial
while(i <= maxiter & deltaJ > 0.000001){ 
  theta <- theta - alpha * gradJ(theta)
  monitor[(i+1), ] = c(i, theta, J(theta), gradJ(theta), alpha)
  deltaJ <- abs(monitor$J[i+1]-monitor$J[i])
  i <-  1 + i
}
```
...
Estimación de theta: `r monitor[nrow(monitor),"theta0"]` y `r monitor[nrow(monitor),"theta1"]`

Coste mínimo alcanzado: `r monitor[nrow(monitor),"J"]`
