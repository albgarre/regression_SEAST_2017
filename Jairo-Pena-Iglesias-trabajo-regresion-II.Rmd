...
rows <- c(1 : nrow(GradosTeleco))
training.posiciones <- sample(rows, nrow(GradosTeleco)*0.7)
...
test.posiciones <- which(! (row.names(GradosTeleco) %in% training.posiciones)) #cogemos los valores que NO pertenecen al conjunto de prueba (training.posiciones)

## hemos usado la negación lógica que es !, 
## (es decir !FALSE es TRUE, y !TRUE es FALSE)

## **%in%** operador lógico que permite comprobar la pertenencia a un vector de valores
```
...
```{r}
## Construimos el vector GradosTeleco.test
GradosTeleco.test <- GradosTeleco[test.posiciones,] #recuerda que termina en coma para que cojamos toda la fila
```
...
##Variable = notamedia
##Característica = asignaturas de la PAU
##he tomado los valores de la PAU que me proporcionan una nota media
```
...
```{r}
## Creamos el data.frame test.predict.PAU
...
##al valor de la notamedia de los datos GradoTeleco le resto mi prediccion con los datos que supuse que obtendria tras analizar solo la PAU
```
...
plot + geom_density(fill = "blue", alpha = 0.5)
```
...
resumenerrores <- errores %>% 
                  ...
                  summarise(error_cuadratico = mean(errores^2, na.rm = TRUE), ##1
                         p0.25 = sum(abs(errores) < 0.25, na.rm = 
                                       TRUE) / length(errores) * 100, ##2
                         p0.1 = sum(abs(errores) < 0.1, na.rm = 
                                      TRUE) / length(errores) * 100) ##3
```
...
test.predict.PAU.primeroscursos <- predict(training.lm.PAU.primeroscursos, GradosTeleco.test)
...
plot <- errores  %>% 
        ggplot(aes(x = errores))
plot + geom_density(aes(fill = caracteristicas), alpha = 0.5)
```
...
##Variable = notamedia
##Característica = asignaturas de la carrera
##he tomado los valores de las notas de la PAU + las notas de las asignaturas de la carrera que me proporcionan una nota media, para lo que he añadido ahora las variables de la asignatura T.Comunicacion
```
...
##ajuste = training.lm.completo
##individuos del conjunto test = GradosTeleco.test
...
No, ya que aumentan las variables pero no llegamos a obtener un dato más fiable.
