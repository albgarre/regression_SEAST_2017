...
```{r}
## Completar aqui"
GradosTelecoSimple %>% ggplot(aes(x =PAU_CALIFICACION_NOTA_MATERIA, y = notamedia)) + 
  geom_point()
```
...
```{r}
# Completar aquí
GradosTelecoSimple %>% ggplot(aes(x =PAU_CALIFICACION_NOTA_MATERIA_DECIL, y = notamedia)) + 
  geom_point()
```
...
```{r}
J <- function(theta){
  #theta <- theta - 2 / length(y) * alpha * t(X) %*% (X %*% theta - y)
...
for (i in 1:maxiter)
  {
  theta <- theta - alpha * gradJ(theta)
  monitor[j,]=c(j-1, theta, J(theta), gradJ(theta), alpha)
  j <- j+1
}
```
...
monitor %>%  filter(iter>=50) %>%
  ggplot(aes(y = J, x = iter, color=alpha)) + 
  geom_line()
```
