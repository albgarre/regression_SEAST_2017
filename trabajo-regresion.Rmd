...
ggplot(data = monitor, aes(x = iter, y = J)) + geom_line(colour ='red', size = 1)
```
...
monitor %>% 
  ...
  ggplot(aes(x = iter, y = J)) + geom_line(colour ='red', size = 1)
...
Si miramos las últimas filas vemos que los valores van creciendo muy lentamente, como nos muestra la representación.  
...
monitor %>% 
  filter (iter > 50) %>% 
  ggplot(aes(x = iter, y = J, colour = alpha)) + geom_line(size = 1)
...
```{r}
monitor %>% 
  ggplot(aes(x = iter, y = J)) + geom_line(size = 1)
