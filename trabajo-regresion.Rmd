...
```{r}
## Completar aqui"
GradosTelecoSimple <-  read.table("data/notamediagradosETSIT-simple.csv", header = TRUE, sep = ",", dec = ".")
```
...
Definir una variable y que contenga los valores de notamedia, y la matriZ X que sea la matriz de diseño asociada a la fórmula: 
...
for(i in 2:maxiter){
  theta <- theta - alpha * gradJ(theta)
  monitor[i,]=c(i-1, theta, J(theta), gradJ(theta), alpha)
}
```
...
```{r}
## Completar aquí
ggplot(data=monitor,aes(x=iter, y=J)) + geom_line(colour='Red')
```
...
```{r}
## completar aquí...
monitor %>% filter(iter>=50) %>% ggplot(aes(x=iter, y=J)) + geom_line(colour='Red')
```
...
monitor <- data.frame(iter = NA, theta0 = NA, theta1 = NA,
                      J = NA, gradJ0 = NA,
                      gradJ1 = NA, alpha = alpha)
monitor[1,] <- c(0, thetainicial, 0, gradJ(thetainicial), alpha)
```
...
for(i in 1:maxiter){
  theta <- theta - alpha * gradJ(theta)
  monitor[j,]=c(i-1, theta, J(theta), gradJ(theta), alpha)
  j <- j+1
}
}
```
...
monitor %>% filter(iter>=50) %>% ggplot(aes(x=iter, y=J, color= alpha))+ geom_line() 
```
...
ggplot(monitor, aes(x=iter, y=J))+ geom_line() 
```
...
while(i<=maxiter){
  ...
  theta <- theta - alpha * gradJ(theta)
  monitor[j,]=c(i-1, theta, J(theta), gradJ(theta), alpha)
  deltaJ = abs(monitor$J[j]- monitor$J[j-1])
  j <- j+1
  i <- i+1
}
