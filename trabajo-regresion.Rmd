...
```{r}
## Completar aqui"
ggplot(GradosTelecoSimple, aes(x = PAU_CALIFICACION_NOTA_MATERIA, y = notamedia)) +
  geom_point()
```
...
```{r}
# Completar aquí
ggplot(GradosTelecoSimple, aes(x = PAU_CALIFICACION_NOTA_MATERIA_DECIL,
                               y = notamedia)) +
  geom_point()
```
...
```{r}
## Completar aquí
ggplot(monitor, aes(x = iter, y = J)) + geom_line(color = "red")
```
...
for (j in Valpha)
{
  ...
  for (i in 1:maxiter)
    {
      theta <- theta - j * gradJ(theta)
      monitor[D, ]=c(i, theta, J(theta), gradJ(theta), j)
      D <- D + 1
    }
}
```
...
while(i <= maxiter)
{
  ...
  if (i>1)
  {
    deltaJ <- abs(monitor$J[i] - monitor$J[i-1])
  }
  i <- i + 1
}
```
