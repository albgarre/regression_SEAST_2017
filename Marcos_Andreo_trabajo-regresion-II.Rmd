...
```{r}
## Completar aqui
GradosTeleco <- read.table(
  file="data/notamediagradosETSIT-completo.csv",
                           header = TRUE ,
                           sep = ";", 
                           dec = ".", 
...
## param1: numero de filas, param2: numero de elementos a samplear
...
training.lm.PAU <- lm(data = GradosTeleco.training, 
                      notamedia ~ PAU_CALIFICACION_NOTA_MATERIA + PAU_CALIFICACION_NOTA_MATERIA_DECIL + FISICA_NOTA_MATERIA + FISICA_NOTA_MATERIA_DECIL + MATEMATICAS_II_NOTA_MATERIA + MATEMATICAS_II_NOTA_MATERIA_DECIL)

summary(training.lm.PAU)
```
...
resumenerrores <- errores %>% 
  ...
  summarise (error_cuadratico_medio = mean(errores^2, na.rm = TRUE),
             porcentaje_alum_errormenor_25 = sum(abs(errores) < 0.25, na.rm = TRUE) / length(errores) *100,
             porcentaje_alum_errormenor_10 = sum(abs(errores) < 0.10, na.rm = TRUE) / length(errores) *100)
```
...
summary(training.lm.PAU.primercurso) 
```
...
GradosTeleco.test %>% 
  ...
  ggplot(aes(x = PAU_CALIFICACION_NOTA_MATERIA, y = notamedia)) +
  geom_point(color = "black") +
  geom_point(aes(x = PAU_CALIFICACION_NOTA_MATERIA, y = prediccion, color = "red"))
```
