...
training.posiciones <- sample(c(1:nrow(GradosTeleco)), nrow(GradosTeleco)*0.7)
...
resumenerrores <- errores %>% 
  ...
  summarise (error_cuadratico_medio = mean(errores^2, na.rm = TRUE),
             porcentaje25 = sum(abs(errores) < 0.25, na.rm = TRUE) / length(errores) *100,
             porcentaje10 = sum(abs(errores) < 0.10, na.rm = TRUE) / length(errores) *100)
```
