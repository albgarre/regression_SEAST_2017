...
training.posiciones <- sample(nrow(GradosTeleco),70, FALSE)
...
#mean(errores$errores)^2
#mean(abs(errores$errores) <= 0.25)
#mean(abs(errores$errores) <= 0.10)
resumenerrores <- errores %>% group_by(caracteristicas) %>% 
  summarise(error_cuad_medio = mean(errores$errores^2),
            por_error_abs_25 = mean(abs(errores$errores) <= 0.25),
            por_error_abs_1 = mean(abs(errores$errores) <= 0.10))
...
```{r}
### Completar aquí, usando ggplot con geom_density,
errores %>% ggplot(aes(x=errores,fill=caracteristicas)) + 
...
test.df <- data.frame(PAU_CALIFICACION_NOTA_MATERIA=GradosTeleco.test$PAU_CALIFICACION_NOTA_MATERIA,
                      notamedia=test.predict.completo)

GradosTeleco.test %>% ggplot(aes(x=PAU_CALIFICACION_NOTA_MATERIA, y= notamedia)) + geom_point() + geom_point(data=test.df,color="red")
```
