...
training.posiciones <- row.names(GradosTeleco %>% sample_n(68))
## completar aquí para construir el subconjunto GradosTeleco.training que 
## corresponda a los GradosTeleco escogidos
GradosTeleco.training <- GradosTeleco %>% filter(row.names(GradosTeleco) %in% training.posiciones)
```
...
GradosTeleco.test <- GradosTeleco %>% filter(row.names(GradosTeleco) %in% test.posiciones)
```
...
training.lm.PAU <- lm(data = GradosTeleco.training,
                      notamedia ~ PAU_CALIFICACION_NOTA_MATERIA +
                                  PAU_CALIFICACION_NOTA_MATERIA_DECIL + 
                                  FISICA_NOTA_MATERIA + 
                                  FISICA_NOTA_MATERIA_DECIL +
                                  MATEMATICAS_II_NOTA_MATERIA +
                                  MATEMATICAS_II_NOTA_MATERIA_DECIL)
```
...
errores %>% ggplot(aes(errores), color = "blue") +
...
resumenerrores <- errores %>% 
  ...
  summarise(error_cuadratico_medio = mean(errores * errores, na.rm = TRUE),
            porcentaje_cuarto = length(which(abs(errores) <= 1/4)) / length(errores) * 100,
            porcentaje_decima = length(which(abs(errores) <= 1/10)) / length(errores) * 100)
resumenerrores
```
...
training.lm.PAU.primercurso <- lm(data = GradosTeleco.training, 
                                  notamedia ~ PAU_CALIFICACION_NOTA_MATERIA +
                                    PAU_CALIFICACION_NOTA_MATERIA_DECIL +
                                    FISICA_NOTA_MATERIA +
                                    FISICA_NOTA_MATERIA_DECIL +
                                    MATEMATICAS_II_NOTA_MATERIA +
                                    MATEMATICAS_II_NOTA_MATERIA_DECIL +
                                    FUNDAMENTOS_DE_PROGRAMACION_NOTA +
                                    FUNDAMENTOS_DE_PROGRAMACION_CONVOCS_PRESENTADAS_ACUMULA +
                                    FUNDAMENTOS_DE_PROGRAMACION_CONVOCS_HASTA_1RA_VEZ_ACUMULA +
                                    SISTEMAS_Y_CIRCUITOS_NOTA +
                                    SISTEMAS_Y_CIRCUITOS_CONVOCS_PRESENTADAS_ACUMULA +
...
test.predict.PAU.primeroscursos
```
...
errores.predict.PAU.primeroscursos <- GradosTeleco.test$notamedia -
  test.predict.PAU.primeroscursos
```
...
```{r}
### Nada que completar, pero sí comprobar que entendéis el código
errores <- rbind(data.frame(caracteristicas = "PAU y primer curso",
...
```{r}
### Completar aquí, usando ggplot con geom_density,
errores %>% ggplot(aes(errores, fill = caracteristicas)) +
...
```{r}
### Completar aquí
errores <- rbind(data.frame(caracteristicas = "Completo",
...
Sí pues el margen de error se vuelve mucho menor.
...
GradosTeleco.test %>% ggplot(aes(x = PAU_CALIFICACION_NOTA_MATERIA,
                                 y = notamedia)) +
  geom_point() +
  geom_point(data = data.frame(notamedia = test.predict.completo,
                               PAU_CALIFICACION_NOTA_MATERIA =
                                 GradosTeleco.test$PAU_CALIFICACION_NOTA_MATERIA), color = "red")
```
