...
x<-1:98
training.posiciones<-sample(x,70)
...
resumenerrores<-errores%>%group_by(caracteristicas)%>%summarise(error_cuadratico=mean(errores^2), menor_cuarto=sum(abs(errores)<=0.25,na.rm=TRUE)/length(errores),menor_decima=sum(abs(errores)<=0.1,na.rm=TRUE)/length(errores))
  ```
...
```{r}
### Completar aquí, usando ggplot con geom_density,
ggplot(errores,aes(errores,fill=caracteristicas)) +geom_density(alpha=0.5)
```
