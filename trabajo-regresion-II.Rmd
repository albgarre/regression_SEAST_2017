...
errores.predict.PAU <- GradosTeleco$notamedia[test.posiciones] -
  test.predict.PAU
```
...
ggplot(errores, aes(errores)) + geom_density(fill = "blue", alpha = 0.5)
```
...
resumenerrores <- errores %>%
  ...
  summarise(errorcuad = mean(errores^2, na.rm = TRUE),
            porcent4 = sum(abs(errores)<=0.25, na.rm = TRUE)/length(errores),
            porcent1 = sum(abs(errores)<=0.1, na.rm = TRUE)/length(errores))
...
training.lm.PAU.primeroscursos <- lm(data = GradosTeleco.training,
                      notamedia ~ PAU_CALIFICACION_NOTA_MATERIA +
                        PAU_CALIFICACION_NOTA_MATERIA_DECIL +
                        FISICA_NOTA_MATERIA +
                        FISICA_NOTA_MATERIA_DECIL +
...
test.predict.PAU.primeroscursos <- predict(training.lm.PAU.primeroscursos,
                                           GradosTeleco.test)
```
...
```{r}
### Completar aquí, usando ggplot con geom_density,
ggplot(errores, aes(errores, fill = caracteristicas)) +
...
training.lm.completo <- lm(data = GradosTeleco.training,
                      notamedia ~ PAU_CALIFICACION_NOTA_MATERIA +
                        ...
                        SISTEMAS_Y_CIRCUITOS_CONVOCS_PRESENTADAS_ACUMULA +
                        SISTEMAS_Y_CIRCUITOS_CONVOCS_HASTA_1RA_VEZ_ACUMULA +
                        TEORIA_DE_LA_COMUNICACION_NOTA +
                        TEORIA_DE_LA_COMUNICACION_CONVOCS_PRESENTADAS_ACUMULA +
...
errores.predict.completo <- GradosTeleco.test$notamedia - 
  test.predict.completo
```
...
GradosTeleco.test$test.predict <- test.predict.completo
ggplot(GradosTeleco.test, aes(x = PAU_CALIFICACION_NOTA_MATERIA,
                              y = notamedia)) +
  geom_point() +
  geom_point(aes(y = test.predict),
             color = "red")
```
